...
st.title("🏆 GitHub Gist ランキング")
st.caption("データはGitHub Gistに保存され、全員で共有されます。")
...
def get_ranking_from_gist():
    try:
        ...
        file = gist.files['ranking.json']
        content = file.content
        
        data = json.loads(content)
        return data
    except Exception as e:
        st.error(f"Gist読み込みエラー: {e}")
        return []
...
def save_ranking_to_gist(new_data):
    try:
        ...
        # 更新
        gist.edit(files={'ranking.json': {"content": json.dumps(new_data)}})
        return True
    except Exception as e:
        st.error(f"Gist保存エラー: {e}")
        return False
...
<script>
  ...
  function playSound(type) {{
    ...
    if (type === 'jump') {{ osc.frequency.setValueAtTime(150, now); osc.frequency.linearRampToValueAtTime(300, now+0.1); gain.gain.exponentialRampToValueAtTime(0.01, now+0.1); osc.start(now); osc.stop(now+0.1); }}
    else if (type === 'coin') {{ osc.frequency.setValueAtTime(1200, now); gain.gain.exponentialRampToValueAtTime(0.01, now+0.2); osc.start(now); osc.stop(now+0.2); }}
    else if (type === 'hit') {{ osc.type='sawtooth'; osc.frequency.setValueAtTime(100, now); gain.gain.exponentialRampToValueAtTime(0.01, now+0.3); osc.start(now); osc.stop(now+0.3); }}
  }}
