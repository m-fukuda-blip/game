...
# ==========================================
# ⚠️ 設定エリア：あなたのURLを貼ってください
# ==========================================
# ブラウザのアドレスバーにあるURLをコピーして、下の "" の中に貼ってください
APP_URL = "https://379e3dtthewydkexmys3au.streamlit.app/" 

st.set_page_config(page_title="Gist Ranking Game", layout="wide")
st.title("🏆 GitHub Gist ランキング (修正完了版)")
st.caption("雲の形とリロード処理を修正しました")
...
if 'new_score' in qp and 'new_name' in qp:
    ...
    if save_ranking_to_gist(current_data):
        st.toast(f"🎉 記録しました！ {new_name}: {new_score}")
...
<script>
  ...
  function submitScore() {{
    ...
    try {{
        ...
        console.error(e);
        alert("リロードエラー: " + e.message);
    }}
  }}
  
  function reloadGame() {{
    if (!appUrl) {{ location.reload(); return; }}
